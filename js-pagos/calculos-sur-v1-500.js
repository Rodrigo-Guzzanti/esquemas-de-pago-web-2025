document.addEventListener('DOMContentLoaded', function()  {
    ...
    const url = 'https://script.google.com/macros/s/AKfycbwjkHUzcJ4nlQ_oTw5kEN-KMLfN9uJLc3sWhixJNBqjaxN9bydyB-B5F-YWC7Sn219eQQ/exec?hoja=Sur%20v1%20CODIGO';
    ...
    function calcularValorDinamico1(distancia) {
        ...
        if (distancia <= 1.5) {
            amount = distancia * tarifasValores[13].tarifasValores1; // Usar tarifa de la primera barra
        } else if (distancia <= 2.5) {
            amount = ((distancia - 1.5) * tarifasValores[14].tarifasValores1) + (1.5 * tarifasValores[13].tarifasValores1);
        } else if (distancia <= 4) {
            amount = ((distancia - 2.5) * tarifasValores[15].tarifasValores1) + (1 * tarifasValores[14].tarifasValores1) + (1.5 * tarifasValores[13].tarifasValores1);
        } else if (distancia <= 6) {
            amount = ((distancia - 4) * tarifasValores[16].tarifasValores1) + (1.5 * tarifasValores[15].tarifasValores1) + (1 * tarifasValores[14].tarifasValores1) + (1.5 * tarifasValores[13].tarifasValores1);
        }
    ...
    function calcularValorDinamico2(distancia) {
        ...
        if (distancia <= 2) {
            amount = distancia * tarifasValores[13].tarifasValores2; // Usar tarifa de la primera barra
        } else if (distancia <= 4) {
            amount = ((distancia - 2) * tarifasValores[14].tarifasValores2) + (2 * tarifasValores[13].tarifasValores2);
        } else if (distancia <= 6) {
            amount = ((distancia - 4) * tarifasValores[15].tarifasValores2) + (2 * tarifasValores[14].tarifasValores2) + (2 * tarifasValores[13].tarifasValores2);
        } else if (distancia <= 8) {
            amount = ((distancia - 6) * tarifasValores[16].tarifasValores2) + (2 * tarifasValores[15].tarifasValores2) + (2 * tarifasValores[14].tarifasValores2) + (2 * tarifasValores[13].tarifasValores2);
        }
    ...
    function renderTable1() {
        ...
        rowsToShow.forEach((record, index) => {
            ...
            if (index === diasSemana.length ) { // Asegurar que se coloca en el inicio del grupo
        ...
        summaryRow.classList.add('summary-row-car');
        const adicional = document.createElement('td');
        adicional.setAttribute('colspan', 4);
        adicional.textContent = '🚗 Adicional punto de retiro con auto +$500';
    ...
    function renderTable2() {
        ...
        rowsToShow.forEach((record, index) => {
            ...
            // Si estamos en la fila donde empieza Grupo 1, agregamos la celda "Adicional por grupo"
            if (index === tramos.length - 4) { // Asegurar que se coloca en el inicio del grupo
                const adicionalCell = document.createElement('td');
                adicionalCell.textContent = 'Adicional por grupo';
                adicionalCell.setAttribute('rowspan', 4); // Se extiende a los cinco grupos
                adicionalCell.classList.add('adicional-cell');
